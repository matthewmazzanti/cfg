-- Render a-z (and buffer-local A-Z) marks in the sign column, driven entirely by
-- events -- no polling timer, zero idle cost. Rendering is a plain loop over
-- getmarklist(); the interesting part is knowing when to re-run it:
...
-- All funnel through one schedule-once reconcile, so a burst of triggers collapses
-- to a single redraw. We always re-derive from getmarklist rather than trusting a
-- placed extmark to track the mark: :sort keeps a-z marks at their line numbers but
-- shoves a generic extmark elsewhere, so the two desync without a re-derive.
...
-- Attach + initial paint. BufReadPost/BufNewFile catch (re)loads -- a plain :edit
-- detaches the buffer callbacks but doesn't fire BufEnter (you never left) -- and
-- BufEnter catches buffer switches. attach() is guarded, so overlap is cheap.
...
-- Cover buffers already loaded when this config loads -- a live :source, or a
-- restored session with several buffers open. (Args like `nvim foo bar baz` load
-- only the first now; the rest are unloaded until visited, where BufEnter takes
-- over.) attach()/schedule() are guarded, so revisiting these is cheap.
